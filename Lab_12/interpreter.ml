...
let rec exec (s: stmt) (stt: state) : state =
  ...
  | IfThen (cond, s) -> exec (IfThenElse(cond, s, Skip)) stt
  | While (cond, body) ->
  ...
  | Skip -> stt
  (*| For (x, e1, e2, s1) -> (match eval e1 stt, eval e2 stt with
                          | Int v1, Int v2 -> if v1 <> v2 then
                                              exec s (exec s1 stt)
...
let val_sum_evens = Int 56
let val_sum_odds = Int 49
let num_sum_evens = 9
let num_sum_odds = 8
